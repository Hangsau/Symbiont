...
def _parse_frontmatter(content: str) -> dict:
    """從 Markdown frontmatter 解析 valid_until、review_by 和 tier 欄位。"""
    match = re.match(r"^---\s*\n(.*?)\n---\s*\n", content, re.DOTALL)
    if not match:
        return {}
    ...
    for line in match.group(1).splitlines():
        ...
        key = key.strip()
        if key in ("valid_until", "review_by", "tier"):
...
def _enforce_tier_index(
    memory_dir: Path,
    index_path: Path,
    dry_run: bool,
) -> int:
    """從 MEMORY.md 移除 tier: L2 的條目（保留 .md 檔案，只清索引）。
    tier 欄位缺失視為 L1（遷移期保守行為，不誤刪舊記憶）。
    回傳移除條數。
    """
    content = safe_read(index_path)
    if not content:
    ...
    for line in content.splitlines():
        ...
        if not md_path.exists():
            continue
        file_content = safe_read(md_path)
        if not file_content:
            continue
        tier = _parse_frontmatter(file_content).get("tier", "L1")
        if tier == "L2":
    ...
    if dry_run:
        print(f"  [dry-run] tier L2 索引條目將移除（{len(to_remove)} 條）：")
        for p in to_remove:
            print(f"    - {p}")
...
def run(dry_run: bool = False) -> int:
    ...
    try:
        ...
        # ── 3.2. Tier 強制執行：移除 L2 條目 ────────────────────────
        tier_removed_count = 0
        if tier_enabled:
            tier_removed_count = _enforce_tier_index(memory_dir, index_path, dry_run)
            if tier_removed_count and not dry_run:
                index_content = safe_read(index_path)
                memory_lines = len(index_content.splitlines()) if index_content else 0
        ...
        if tier_removed_count:
            lines.append(f"  ✓ Tier L2 條目從索引移除：{tier_removed_count} 條（.md 檔案保留）")
        if pruned_count:
