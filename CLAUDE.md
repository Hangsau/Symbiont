...
│   ├── synthesize.py       # 跨 session 批次分析 → skill 生成 / memory 寫入
...
│   ├── synth_state.json    # synthesis 計數器 + skill 使用率統計（gitignore）
...
| `synthesize.py` | 最近 N 個 session 的 friction + habit 片段 | `~/.claude/skills/` 新建或迭代 skill、memory/thoughts/ 洞見、低使用率 skill 清掃 | 由 evolve.py 計數觸發（每 10 次 session） |
