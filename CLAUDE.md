...
**排程任務**：用戶自訂的定期 `claude -p` 任務定義在 `config.yaml` 的 `user_jobs` 區塊。新增排程請用 `/add-scheduled-job` skill。
...
│   ├── session_wrap.py     # post-session 自動補跑 wrap Step 0（Memory Audit）和 Step 2（Reflect 洞見）
│   ├── babysit.py          # agent 保母：poll inbox → 決策 → 回應（teaching/discussion 雙 mode）
│   ├── healthz.py          # CLI：讀 heartbeat.json 回報 babysit 健康狀態
│   ├── user_scheduler.py   # 用戶自訂排程任務：讀 config.yaml user_jobs → cron 比對 → cooldown → 執行 claude -p
│   └── utils/
...
│       ├── claude_runner.py       # claude -p subprocess 封裝（含 cwd 參數支援）
...
│   ├── session_wrap_state.json # session_wrap cursor（自動產生）
...
│   ├── last_user_job_<name>_ts.txt  # run_user_jobs.py 寫入：per-job cooldown timestamp（自動產生）
│   ├── user_jobs.log           # user_scheduler.py 執行記錄（自動產生）
│   ├── pending_session_wrap.txt # Stop hook 寫入：觸發 run_session_wrap.py（自動產生）
│   └── agents.yaml             # agent registry（gitignore，從 agents.example.yaml 複製）
├── scripts/
│   ├── trigger-evolve.py      # Stop hook：寫三個 pending .txt 旗標，純檔案操作（含 pending_session_wrap.txt）
...
│   ├── run_session_wrap.py    # Task Scheduler wrapper：每 1 分鐘 poll pending_session_wrap.txt，有才跑 session_wrap.py
│   ├── run_user_jobs.py       # Task Scheduler wrapper：每小時觸發，內部 per-job cooldown 控制執行頻率
│   └── symbiont-stop-hook.sh  # Stop hook 腳本（Mac/Linux 安裝用）
├── skills/
│   └── add-scheduled-job/
│       └── SKILL.md           # 新增 user_jobs 排程的 Claude skill（setup 安裝到 ~/.claude/skills/）
├── vm-bootstrap/
...
| `user_scheduler.py` | config.yaml `user_jobs` 區塊 | claude -p 執行各 job；`data/last_user_job_<name>_ts.txt` cooldown；`data/user_jobs.log` | 每小時（`scripts/run_user_jobs.py`，pythonw.exe）；per-job cron + cooldown 雙重過濾；pipeline 任一 step 失敗→停止不寫 cooldown |
...
- **evolve.py / synthesize.py / session_wrap.py 輸出格式解析失敗時，不能寫任何檔案**：fallback 只記 error log，寧可不執行也不亂寫；session_wrap.py schema 驗證失敗時寫 `memory/_malformed/`（安全網），不寫主 memory 目錄
...
| Memory Audit（wrap step 0）| wrap 步驟 0（手動） | session_wrap.py | 已遷移（手動覆蓋仍可） |
| reflect 洞見 | wrap 步驟 2 | session_wrap.py | 已遷移（手動覆蓋仍可） |
