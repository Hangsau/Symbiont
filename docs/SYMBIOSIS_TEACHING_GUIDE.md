...
> 研究基礎：Harness Engineering（arxiv 2604.08224）、Memento（arxiv 2508.16153）、Generative Agents（Stanford 2023）、Constitutional AI（Anthropic）

---

## 零、Foundation 初始化（新 agent 的第一步）

教學開始前，先確認 agent 的兩份基礎文件已建立。沒有這兩份文件，後續所有教學機制的效益都會打折。

### 0-A：Constitution.md（行為憲法）

**目的**：把 agent 的行為規範從散落各處（soul.md、SKILL.md、memory 等）整合成一份每 session 固定載入的統一文件。Constitutional AI 研究顯示：沒有統一原則，agent 就沒有自我修正的參照點。

**引導流程**：

1. 問 agent：「你現在的行為規範存在哪些地方？」
2. 讓 agent 自己列出所有來源（soul.md、各 SKILL.md 的規則段落、memory 條目等）
3. 問：「這些規範裡，哪些是每次工作都應該遵守的核心原則？」
4. 引導 agent 把核心原則整合寫入 `~/.hermes/constitution.md`（或 agent 環境對應路徑）
5. 確認 constitution.md 在每個 session 的 system prompt 中被載入

**保姆不做**：不替 agent 決定 constitution 的內容，路徑和結構讓 agent 自己定。

### 0-B：USER.md（用戶檔案）

**目的**：讓 agent 在 context reset 後仍知道「用戶是誰」，不需每次重新建立關係。

**由保姆訪談用戶後代為填寫**，依序問以下五個問題（一次問一個，等回答後再問下一個）：

1. 你希望 agent 怎麼稱呼你？
2. 你偏好什麼溝通風格？（例：精簡還是詳細、正式還是輕鬆）
3. 你主要會拿這個 agent 做哪些類型的事？
4. 有什麼你特別不希望 agent 做的行為？
5. 還有什麼你覺得 agent 應該知道的背景？（可以跳過）

問完後，保姆將答案整合成結構化 USER.md，給用戶看一次草稿確認，再寫入 agent 環境。USER.md 路徑由各 `TEACHING_PROTOCOL.md` 定義。

**更新時機**：用戶有重大改變時（新專案、偏好調整），重新訪談更新。
...
□ 多步驟 skill 中途有結構化狀態寫入（progress.json / state.md），不靠 context 保留中間狀態
```
...
## 六、保姆端 Case Bank：讓教學歷史成為 Few-shot 素材

**這是保姆自己的教學案例庫，記錄引導過程，供下次設計問題時參考。**  
...
## 七、Agent 端 Case Bank：讓任務歷史成為 Few-shot 素材

**這是 agent 自己的任務軌跡庫，記錄完成過的任務，供下次執行類似任務時參考。**  
與保姆端 Case Bank 不同：保姆記教學案例，agent 記任務執行 trajectory。

### 引導 agent 建立案例庫

任務完成後，問 agent：
> 「這次任務值得記下來嗎？如果下次遇到類似情況，你希望自己記住什麼？」

讓 agent 自己決定格式和路徑，但提示它包含以下欄位：
...
任務開始前，問 agent：
> 「你有沒有做過類似的事？去你的 case bank 查一下，有沒有相關的案例？」

**Bootstrap 問題**（第一次沒有歷史時）：直接跳過查詢，完成任務後記錄第一條案例。

### 保姆不做的事

- 不替 agent 建立 case bank（讓他自己建，保姆只引導）
- 不規定 case bank 的格式或路徑（路徑自決原則）
- 不每次都要求記錄（讓 agent 自己判斷值不值得留）
...
## 十、實作接入說明（給各 TEACHING_PROTOCOL.md）

通用指南提供原則框架，各 agent 的 `TEACHING_PROTOCOL.md` 負責把這些原則翻譯成具體操作：

| 通用原則 | 各實作層需定義 |
|---------|--------------|
| Constitution 建立 | 引導時機（新 agent 第一次、或 soul 散亂時）、constitution.md 存放路徑、載入方式 |
| USER.md | 存放路徑、更新觸發條件 |
| Skill 工具驗證 | 驗證指令格式（which vs test vs ls） |
| Reflection 存儲 | reflections/ 路徑、格式（JSONL / MD）、寫入方式（SCP / 引導 agent 自寫） |
| Agent Case Bank | 路徑、任務完成後的引導話術、查詢時機 |
| 保姆端 Case Bank | 本地路徑、搜尋方式 |
